...
集卡赢1元家电好物互助
入口：[京东App领券频道]
25 6,10,14,18,20 * * * Sami_jd_ddqkj_help0420.js
============Sami===============
1、由于签名限制,每次互助都要获取签名,如果担心风险,请禁用该脚本。
*/

const $ = new Env("Sami集卡赢1元家电好物互助")
const Ver = '20220408';
...
!(async () => {
    await $.wait(1000);
    await VerCheck("ddqkjhelp",Ver);
    //console.log(process.env.JD_WSCK)
    await $.wait(1000);
    UUID = getUUID();
    await $.wait(1000);
    requireConfig()
    ...
    console.log(`\n***********开始助力******`);
        for (let i = 0; i < cookiesArr.length; i++) {
            ...
            $.index = i + 1;
            $.UserName1 =encodeURIComponent($.UserName)
            for (jj=0;jj<shareCodes.length;jj++){
                console.log(`\n账号【${$.index}】${$.UserName} 开始助力${shareCodes[jj]}******`);
                await help($.UserName1,activitykey,shareCodes[jj])
                await $.wait(2000)
                if (datastr.data?.biz_msg=="今日TA的助力已满，去完成自己的活动吧"){
                    return;
                }
                if (datastr.data?.biz_msg=="今日助力机会已用完，去完成自己的活动吧"){
                    break;
                }
            }
...
//点点券
async function getHelpNo(){
    try{
        let data = await GetAllTask("necklacecard_openGroup","","body=%7B%22activityKey%22%3A%22"+activitykey+"%22%7D");
        console.log("😜😜😜助力码："+data.data?.result?.groupId)
        shareCodes.push(data.data?.result?.groupId)
    }catch(e){
...
async function help(UserName1,activitykey,id){
    try{
        await getBodySign('necklacecard_assist',$.UserName1,activitykey,id,'','');
        let data = await startTask("necklacecard_assist","");
        datastr=data;
