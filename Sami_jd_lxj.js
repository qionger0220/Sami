...
3、脚本不提供默认执行时间,避免同时段获取大量签名导致执行失败,请自行设置执行时间
 */
        ...
        if (cookie) {
            ...
            if(data.code===0){
                ...
                for (let vo of data.data.result.taskInfos){
                    if(vo.type===4){ //逛商品
                         getSign=await getBodySign(vo.desc, $.UserName)
                          if (getSign.code === 100){
...
/*function getBodySign(sku,UserName){
     return new Promise(resolve => {
        $.get({
            url:`http://199.101.171.13:1880/jd_lxj?sku=${sku}`,
            headers: {
                "Cookie": cookie,
                "pt_pin": UserName
            },
